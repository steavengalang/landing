...
export default async function handler(req, res) {
  ...
  if (tier === 'pro' && licenseKey) {
    try {
      ...
      if (!licenseDataStr) {
        return res.json({ 
          usage: 0, limit: 3, remaining: 3, canUse: false, 
          downgraded: true, reason: 'License not found'
        });
      }
      ...
      if (license.status === 'revoked') {
        return res.json({ 
          usage: 0, limit: 3, remaining: 3, canUse: false, 
          downgraded: true, reason: 'License revoked'
        });
      }
      ...
      if (new Date() > new Date(license.expiresAt)) {
        return res.json({ 
          usage: 0, limit: 3, remaining: 3, canUse: false, 
          expired: true, reason: 'License expired'
        });
      }

      const today = new Date().toISOString().split('T')[0];
      await redis.incr(`pro-usage:${licenseKey}:${today}`);
      await redis.expire(`pro-usage:${licenseKey}:${today}`, 86400 * 30);

      return res.json({ 
        usage: 0, limit: -1, remaining: -1, canUse: true, tier: 'pro'
      });
      
    } catch (error) {
      console.error('Pro verification error:', error);
      return res.json({ 
        usage: 0, limit: 3, remaining: 0, canUse: false, error: 'Verification failed'
      });
    }
  }

  // Free tier - CHECK BEFORE INCREMENT (FIX)
  try {
    const redisUrl = process.env.KV_REST_API_URL || process.env.UPSTASH_REDIS_REST_URL;
    const redisToken = process.env.KV_REST_API_TOKEN || process.env.UPSTASH_REDIS_REST_TOKEN;
    ...
    if (usage >= limit) {
      return res.json({ 
        usage, limit, remaining: 0, canUse: false, tier: 'free'
      });
    }
    ...
    return res.json({ 
      usage: newUsage, limit, remaining, canUse: true, tier: 'free'
    });
  } catch (error) {
